...
数据快照为 2026-08-13，覆盖 14 门实际开课课程、17 个课程—学期组合、127 个 AIMS CRN 和 160 段上课时间，并保留培养方案内未查到开课记录的 MS6712。网站允许选择未注明 `only for Programme` 的公开班次，以及限制名单包含 `MSCBDA3` 的班次；明确只限其他 Programme 的班次不可选。当前快照的 127 个 CRN 均有 Programme 限制，其中 41 个包含 `MSCBDA3`。同一 CRN/Section 若每周有多节课，全部日期、时间与教室都会一并显示。余位只反映快照，不是实时数据；教师、时间、地点与注册状态均应以 CityU AIMS 为准。
