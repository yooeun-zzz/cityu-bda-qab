...
open_crns={(x["term"],m["crn"]) for x in actual for m in x["meetings"] if m["eligibilityReason"]=="open"}
assert len(open_crns)==0, "Current snapshot should have no unrestricted CRNs"
for x in actual:
 ...
 for m in x["meetings"]:
  ...
  restricted=bool(re.search(r"only for Programme\s*:",m["registrationRestrictions"],re.I))
  includes=bool(re.search(r"\bMSCBDA3\b",m["registrationRestrictions"],re.I))
  assert m["eligibleForProgramme"] == (not restricted or includes)
  assert m["eligibilityReason"] == ("mscbda3" if includes else ("open" if not restricted else "restricted_other"))
...
print(f"PASS: 14 offered courses, 17 course-term combinations, {len(crns)} CRNs, {len(eligible_crns)} eligible CRNs ({len(open_crns)} unrestricted), {len(meetings)} meeting times; multi-meeting Sections preserved; MS6712 unavailable")
